...
namespace Opsive.Shared.StateSystem
{
    ...
    public class StateManager : MonoBehaviour
    {
        ...
        private void InitializeInternal(GameObject gameObject, IStateOwner owner, State[] states)
        {
            ...
            var characterLocomotion = gameObject.GetCachedParentComponent<Character.ICharacter>();
            ...
            for (int i = 0; i < states.Length; ++i) {
                ...
                if (!m_GameObjectNameStateList.TryGetValue(gameObject, out var nameStateList)) {
        ...
        private void SetStateInternal(object owner, State[] states, string stateName, bool active)
        {
            ...
            if (!m_ObjectNameStateMap.TryGetValue(owner, out var nameStateMap)) {
                Debug.LogWarning(string.Format("Warning: Unable to find the name state map on object {0}.", owner));
                return;
            }

            // Lookup the state by name.
            if (!nameStateMap.TryGetValue(stateName, out var state)) {
                Debug.LogWarning(string.Format("Warning: Unable to find the state with name {0}.", stateName));
                return;
            }
        ...
        private void SetStateInternal(GameObject gameObject, string stateName, bool active)
        {
            ...
            var character = gameObject.GetCachedComponent<Character.ICharacter>();
            if (character != null) {
                if (m_ActiveCharacterStates.TryGetValue(gameObject, out var activeStates)) {
        ...
        {
            if (m_LinkedGameObjectList.TryGetValue(gameObject, out var linkedGameObjects)) {
        ...
        private void DeactivateStateTimerInternal(GameObject gameObject, string stateName, float time)
        {
            ...
            if (m_DisableStateTimerMap.TryGetValue(gameObject, out var stateNameEventMap)) {
